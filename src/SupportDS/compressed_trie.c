...
#define ipa_masklen(x) ip6_masklen_t(&x)
 ...
 */
struct f_trie *
f_new_trie(uint data_size)
{
  struct f_trie * ret;
  ret = lp_allocz(sizeof(struct f_trie) + data_size);
  ret->ipv4 = -1;
...
{
  struct f_trie_node4 *n = lp_allocz(sizeof(struct f_trie_node4) + t->data_size);
  n->plen = plen;
  n->local = local;
  n->addr = paddr;
  n->mask = pmask;
...
{
  struct f_trie_node6 *n = lp_allocz(sizeof(struct f_trie_node6) + t->data_size);
  n->plen = plen;
  n->local = local;
  n->addr = paddr;
  n->mask = pmask;
...
void count_node_ip4(struct f_trie_node4 *root,int *res){
  if(root){
    *res = (*res)+1;
    for(int i=0;i<(1<<TRIE_STEP);i++){
      count_node_ip4(root->c[i],res);
    }
  }
}

void count_node_ip6(struct f_trie_node6 *root,int *res){
  if(root){
    *res = (*res)+1;
    for(int i=0;i<(1<<TRIE_STEP);i++){
      count_node_ip6(root->c[i],res);
    }
  }
}
