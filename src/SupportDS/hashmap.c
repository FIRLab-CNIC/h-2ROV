...
void *hashmap_get(struct hashmap *map, const void *key) {
	...
	for (;;) {
        struct bucket *bucket = bucket_at(map, i);
        // __builtin_prefetch(bucket_at(map,(i + 1) & map->mask),0,1);
		if (!bucket->dib) {
			return NULL;
		}
		if (bucket->hash == hash && 
...
void *hashmap_get_cnt(struct hashmap *map, const void *key, double *res) {
    if (!key) {
        panic("key is null");
    }
...
void *hashmap_get_with_hashvalue(struct hashmap *map, const void *key, uint64_t *hash) {
    if (!key) {
        panic("key is null");
    }
	size_t i = *hash & map->mask;
...
void *hashmap_get_show_hash(struct hashmap *map, const void *key) {
    if (!key) {
        panic("key is null");
    }
    uint64_t hash = get_hash(map, key);
...
void *hashmap_get_with_index(struct hashmap *map, const void *key, size_t *index) {
    if (!key) {
        panic("key is null");
    }
    uint64_t hash = get_hash(map, key);
...
void *hashmap_get_by_index(struct hashmap *map, size_t index) {
    struct bucket *bucket = bucket_at(map, index);	
    return bucket_item(bucket);	
}
